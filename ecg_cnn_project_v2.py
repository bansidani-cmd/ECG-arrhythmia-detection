...
REQUIREMENTS (new, in addition to before):
    pip install tensorflow

NOTE: if you've already run the previous scripts, the mitdb/ folder
already has these 44 records downloaded locally, so this should skip
straight to loading without re-downloading (much faster this time).
...
def extract_raw_windows(sig, beat_samples, window=WINDOW_SAMPLES):
    ...
    IMPORTANT FIX: we normalize using the WHOLE RECORDING's mean/std,
    not each beat's own individual mean/std. Normalizing per-beat (our
    first attempt) rescales every beat to the same amplitude range,
    which erases genuine amplitude differences between beats — exactly
    the information our Random Forest found useful (amplitude had real,
    non-trivial feature importance). Normalizing per-recording instead
    keeps the network on a stable numeric scale while preserving how
    tall or short one beat is relative to others in the same recording.
    """
    recording_mean = np.mean(sig)
    recording_std = np.std(sig) + 1e-8
    ...
    for i, peak in enumerate(beat_samples):
        ...
        beat_window = (beat_window - recording_mean) / recording_std
...
# =====================================================================
# Same idea as class_weight='balanced' in Random Forest — tell the network
# to pay more attention to the rare (Abnormal) class during training.
...
plt.tight_layout()
plt.savefig('cnn_training_curves_v2.png', dpi=120)
plt.close()
...
plt.tight_layout()
plt.savefig('cnn_confusion_matrix_v2.png', dpi=120)
plt.close()
